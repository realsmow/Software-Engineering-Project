...
describe('Module 2: User / Account Management (Admin) [BE]', () => {
  ...
  // ── TP-ADM-BE-13: KU Email Domain Restriction ────────────────────────────────
  it('TP-ADM-BE-13: should reject account creation with a non-KU email domain', async () => {
    const timestamp = Date.now();

    await expect(
      adminService.createUser({
        email: `external.${timestamp}@gmail.com`,
        password: 'Password123!',
        studentId: `EXT${timestamp.toString().slice(-6)}`,
        firstName: 'External',
        lastName: 'Domain',
        initialCredit: 100,
        role: 'borrower',
