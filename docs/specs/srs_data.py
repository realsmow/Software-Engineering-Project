...
VERSION = "1.1"
DOC_TITLE = "ULMs Software Requirements Specification v1.1"
RUNNING_HEADER = "ULMs SRS v1.1"
OUTPUT_STEM = "ULMs-SRS-v1.1"
...
BODY = [
    ...
    ('table', [5.6, 10.6],
     ...
     [
        ...
        ['ULMs SDS v1.2', 'เอกสารการออกแบบที่ตอบความต้องการในเอกสารฉบับนี้'],
    ...
    ('table', [7.0, 9.2],
     ['ระบบภายนอก', 'หน้าที่'],
     [
        ['Google OAuth 2.0', 'Authentication บัญชี KU เมื่อผู้ดูแลระบบเปิดใช้ (แก้ไขใน v1.1)'],
        ['Email Service (SMTP/SendGrid/SES)', 'ส่งการแจ้งเตือน'],
        ['ที่เก็บไฟล์บนเซิร์ฟเวอร์ (MEDIA_ROOT)', 'เก็บรูปภาพหลักฐานผ่านตั๋วอัปโหลดที่มีอายุจำกัด และสำรองทุกวัน (แก้ไขใน v1.1)'],
    ]),
    ...
    ('table', [2.4, 13.8],
     ...
     [
        ...
        ['C-02', 'ระบบรองรับการยืนยันตัวตนผ่าน KU Google Workspace เมื่อผู้ดูแลระบบเปิดใช้ และมีบัญชีในระบบที่ยืนยันอีเมล @ku.th เป็นทางหลัก (แก้ไขใน v1.1)'],
    ...
    ('table', [3.4, 10.6, 2.2],
     ...
     [
        ['FR-AUTH-01', 'ระบบต้องรองรับการเข้าสู่ระบบด้วยบัญชีในระบบที่ยืนยันอีเมล @ku.th แล้ว และรองรับ Google OAuth ของบัญชี KU เมื่อผู้ดูแลระบบเปิดใช้ (แก้ไขใน v1.1)', 'สูง'],
    ...
    ('table', [3.4, 10.6, 2.2],
     ...
     [
        ...
        ['FR-REQ-08', 'สำหรับคำขอ T3 Borrower ต้องเลือกสล็อตเวลาละ 30 นาทีที่ต่อเนื่องกัน ได้สูงสุด 6 สล็อต (3 ชั่วโมง) ต่อการจอง และถือการจองห้องที่ยังมีผลได้ครั้งละ 1 รายการ (แก้ไขใน v1.1)', 'สูง'],
    ...
    ('table', [3.4, 10.6, 2.2],
     ...
     [
        ...
        ['FR-APV-05', 'เมื่อคำขอได้รับการอนุมัติ ระบบต้องสร้าง Allocation พร้อมกำหนด pickup_deadline เป็น 1 วันนับจากเวลาเริ่มยืม หรือนับจากเวลาอนุมัติถ้าอนุมัติหลังเวลาเริ่มยืม (แก้ไขใน v1.1)', 'สูง'],
    ...
    ('table', [3.4, 10.6, 2.2],
     ...
     [
        ...
        ['FR-RNW-04', 'สำหรับ T3 ระบบไม่รองรับการต่อเวลา Borrower ที่ต้องการใช้ห้องต่อต้องจองสล็อตถัดไปเป็นการจองใหม่ (แก้ไขใน v1.1)', 'กลาง'],
    ...
    ('table', [3.4, 10.6, 2.2],
     ...
     [
        ...
        ['FR-RTN-03', 'Staff ต้องบันทึกผลตรวจสภาพเป็น 4 ระดับ (B0 ปกติ B1 เสียหายเล็กน้อย B2 เสียหายมาก B3 ชำรุด) พร้อมหมายเหตุ ส่วนของที่ไม่ได้คืนบันทึกเป็นสูญหายแยกจากระดับความเสียหาย (แก้ไขใน v1.1)', 'สูง'],
        ...
        ['FR-RTN-06', 'สำหรับ T3 ระบบต้องเปิดรอบตรวจสภาพสถานที่ให้ Staff ทุก 30 วัน และ Staff ต้องบันทึกผลภายใน 7 วัน (แก้ไขใน v1.1)', 'กลาง'],
        ['FR-RTN-07', 'หากคืนของหลังเวลาปิดทำการของวันครบกำหนด (ค่าเริ่มต้น 17:00 ตั้งได้ตาม FR-ADM-04) ระบบต้องนับเป็นคืนช้า 1 วัน (แก้ไขใน v1.1)', 'สูง'],
    ]),
    ...
    ('table', [3.4, 10.6, 2.2],
     ...
     [
        ...
        ['FR-ADM-04', 'Admin ต้องสามารถตั้งเวลาทำการของเคาน์เตอร์ (work_time ค่าเริ่มต้น 08:00-17:00 เวลาไทย กำหนดคืนคือเวลาปิด) ส่วนระยะเวลายืม จำนวนครั้งที่ต่ออายุ ช่วงคะแนนของระดับเครดิต และอัตราหักเครดิตตั้งได้ในค่าตั้งการยืม และ prep_days ตั้งรายชิ้นโดย Staff (แก้ไขใน v1.1)', 'สูง'],
    ...
    ('table', [3.4, 12.8],
     ...
     [
        ...
        ['NFR-SEC-06', 'ระบบต้องรับรูปภาพหลักฐานผ่านตั๋วอัปโหลดที่เซ็นแล้วและมีอายุจำกัด ตรวจชนิดไฟล์จากเนื้อไฟล์ และตั้งชื่อไฟล์แบบสุ่ม (แก้ไขใน v1.1)'],
    ]),
    ...
    ('table', [4.0, 12.2],
     ['เรื่อง', 'ข้อสรุป (v1.1)'],
     [
        ['ระดับความเสียหาย', 'ใช้ B0-B3 ตามเอกสาร โครงสร้างข้อมูลมีค่าที่ห้าคือสูญหาย ซึ่งใช้กับของที่ไม่ได้คืน ไม่ใช่ระดับความเสียหาย (FR-RTN-03)'],
        ['การจัดระดับอุปกรณ์', 'เหลือวิธีเดียว: ระดับอ่านจากกฎการยืม (BorrowRule) ของแต่ละชิ้น'],
        ['work_time ใน FR-ADM-04', 'นิยามแล้วเป็นเวลาทำการของเคาน์เตอร์ ค่าเริ่มต้น 08:00-17:00 เก็บในค่าตั้งระบบ'],
        ['ที่เก็บรูปหลักฐาน', 'เก็บบนเซิร์ฟเวอร์พร้อมตั๋วอัปโหลดที่มีอายุจำกัด และสำรองรวมกับฐานข้อมูลทุกวัน 03:00 (NFR-SEC-06)'],
    ]),
    ...
    ('h2', '3.7 บันทึกการแก้ไขฉบับ 1.1'),
    ('p',
     'ฉบับ 1.1 ปรับข้อความให้ตรงกับการตัดสินใจของทีมระหว่างพัฒนา ตามขั้นตอนในหัวข้อ 3.6 รหัสความต้องการทุกข้อคงเดิม ข้อที่แก้มีคำว่า "แก้ไขใน v1.1" กำกับไว้'),
    ('table', [3.2, 6.4, 6.6],
     ['รหัส', 'เปลี่ยนอะไร', 'เหตุผล'],
     [
        ['FR-AUTH-01, C-02', 'บัญชีในระบบที่ยืนยันอีเมล @ku.th เป็นทางหลัก Google OAuth เปิดได้เมื่อพร้อม', 'ยังไม่ได้สิทธิ์ KU Google Workspace (A-01) ระบบต้องใช้งานได้ก่อน'],
        ['FR-REQ-08', 'สล็อต 30 นาที สูงสุด 6 สล็อตต่อการจอง ถือได้ 1 การจอง', 'ตารางห้องจริงแบ่งครึ่งชั่วโมง และจำกัดการกักห้องไว้หลายห้อง'],
        ['FR-APV-05', 'กำหนดมารับ 1 วันนับจากเวลาเริ่มยืม แทน 24 ชั่วโมงนับจากเวลาอนุมัติ', 'การจองล่วงหน้าหลายวันจะหมดเวลาก่อนถึงวันรับ'],
        ['FR-RTN-03', 'นิยามระดับ B0-B3 และแยกสูญหายออกจากระดับความเสียหาย', 'ตอบข้อสังเกตในหัวข้อ 3.5'],
        ['FR-RTN-06', 'ตรวจห้องเป็นรอบทุก 30 วัน บันทึกภายใน 7 วัน แทนวันละครั้ง', 'ตรวจทุกวันเป็นภาระเกินกำลังเจ้าหน้าที่ และห้องเปลี่ยนสภาพช้า'],
        ['FR-RTN-07, FR-ADM-04', 'เวลาปิดทำการตั้งได้ ค่าเริ่มต้น 08:00-17:00 และนิยาม work_time', 'ตอบข้อสังเกตในหัวข้อ 3.5 ทีมกำหนดเวลาทำการของบุคลากร 08:00-17:00'],
        ['NFR-SEC-06', 'ตั๋วอัปโหลดที่มีอายุจำกัดบนที่เก็บของเซิร์ฟเวอร์ แทน pre-signed URL บน cloud storage', 'ไม่มีงบ cloud storage (C-05) ความปลอดภัยเทียบเท่าและรวมอยู่ในการสำรองข้อมูล'],
        ['FR-RNW-04', 'ห้อง T3 ต่อเวลาไม่ได้ ให้จองสล็อตถัดไปแทน แทนการต่อทันทีเมื่อสล็อตถัดไปว่าง', 'ทีมตัดสิน การจองใหม่ผ่านการตรวจตารางห้องและเพดาน 3 ชั่วโมงตามปกติ ส่วนการต่อเวลาจะข้ามการตรวจทั้งสอง'],
        ['FR-APV-01', 'ข้อความคงเดิม ส่วนประวัติการยืมในคิวอนุมัติเลื่อนไปรอบถัดไป', 'ตัดสินตามขั้นที่ 3 ของหัวข้อ 3.6'],
        ['ภาคผนวก A', 'ชี้ไปยังรายการ error code จริงในโค้ด', 'รายการในเอกสารไม่ตรงกับรหัสที่ระบบใช้จริง'],
    ]),
    ('h1', 'ภาคผนวก A: Error Code Dictionary'),
    ('p',
     'ตัวอย่าง error code และข้อความภาษาไทยที่ผู้ใช้เห็น รายการจริงทั้งหมดอยู่ที่ frontend/src/lib/error-messages.ts ซึ่งต้องตรงกับรหัสใน backend/src/common/errors/business-error.ts (ตั้งแต่ v1.1)'),
    ('table', [7.6, 8.6],
     ['Code', 'ข้อความภาษาไทย'],
     [
