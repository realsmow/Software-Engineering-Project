...
async function ensureBorrowerIsActive(page: Page) {
  ...
  const lift = panel.getByRole("button", {
    name: "Lift suspension",
    exact: true,
  });
  if (await lift.isVisible().catch(() => false)) {
    ...
    await expect(
      panel.getByRole("button", { name: "Suspend", exact: true }),
    ).toBeVisible();
...
test.describe("Admin borrowing-ban policy", () => {
  ...
  test("TP-ADM-E2E-05: suspends a borrower and restores the seed account afterwards", async ({
    page,
  }) => {
  ...
  test("TP-ADM-E2E-06: lifts a suspension and restores the active status badge", async ({
    page,
  }) => {
    await ensureBorrowerIsActive(page);
  ...
  test("TP-ADM-E2E-07: blocks an administrator from suspending, disabling, or demoting themself", async ({
    page,
  }) => {
    ...
    await closePanel(page);
    await expect(rowFor(page, ADMIN_EMAIL)).toContainText(
      "System administrator",
    );
    await expect(rowFor(page, ADMIN_EMAIL)).toContainText("Active");
  });

  test("leaving a suspended account panel preserves its suspended badge", async ({
    page,
  }) => {
    await ensureBorrowerIsActive(page);
    try {
