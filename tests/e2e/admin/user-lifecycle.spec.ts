...
async function restoreBorrowerRole(page: Page) {
  ...
  if (
    await roleSelect.textContent().then((text) => !text?.includes("Borrower"))
  ) {
...
test.describe("Admin user lifecycle", () => {
  ...
  test("TP-ADM-E2E-01: loads a non-empty account table for an authenticated administrator", async ({
    page,
  }) => {
    await expect(
  ...
  test("TP-ADM-E2E-02: validates the account-creation form without leaving persistent test data", async ({
    page,
  }) => {
    await page
    ...
    await dialog
      .getByPlaceholder("First name – last name")
      .fill("E2E Test User");
  ...
  test("TP-ADM-E2E-03: changes a borrower role and restores the seed role", async ({
    page,
  }) => {
  ...
  test("TP-ADM-E2E-04: exposes password reset for a selected non-admin account", async ({
    page,
  }) => {
    const panel = await openBorrower(page);
    await expect(
      panel.getByRole("button", { name: "Reset password", exact: true }),
    ).toBeEnabled();
    await panel
      .getByRole("button", { name: "Close", exact: true })
      .first()
