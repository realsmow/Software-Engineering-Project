...
export class SessionService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    this.ttlMs =
      Number(this.config.get('SESSION_TTL_HOURS') ?? DEFAULT_TTL_HOURS) * 60 * 60 * 1000;
    ...
    this.cookieOptions = {
      ...
      sameSite: (this.config.get<CookieOptions['sameSite']>('COOKIE_SAMESITE') ??
  ...
  issue(res: Response, accountKey: number): string {
    ...
    res.cookie(SESSION_COOKIE, token, { ...this.cookieOptions, maxAge: this.ttlMs });
    return token;
  }
