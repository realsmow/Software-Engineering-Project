...
const SETTINGS: LendingSettings = {
...
describe('LendingSettingsPage', () => {
  ...
  beforeEach(() => {
    ...
    vi.clearAllMocks();
    useLendingSettingsMock.mockReturnValue({ data: SETTINGS, isLoading: false });
    useUpdateLendingSettingsMock.mockReturnValue({ mutateAsync, isPending: false });
  });

  it('shows a loading state while lending settings are being fetched', () => {
    useLendingSettingsMock.mockReturnValue({ data: undefined, isLoading: true });
  ...
  it('shows the empty state when no borrow rules are configured', () => {
    ...
    expect(screen.queryByRole('button', { name: i18n.t('staff.settings.save') })).not.toBeInTheDocument();
  });
  ...
  it('renders constraint and penalty rows and keeps Save disabled until an edit', () => {
    ...
    expect(screen.getByText(i18n.t('staff.settings.reasonDamagedItem'))).toBeInTheDocument();
